    ...
    <div className="min-h-screen bg-slate-900 px-6 py-12 md:px-12 md:py-20 lg:px-24">
      <div className="max-w-screen-xl mx-auto">
        <header className="mb-12">
          <Link
            to="/"
            className="group inline-flex items-center gap-2 text-sm text-slate-400 hover:text-teal-300 transition mb-6"
          >
            <ArrowLeft
              size={16}
          ...
          <h1 className="text-4xl font-bold tracking-tight text-slate-200 sm:text-5xl">
            {settings.blogTitle || "Writings & Thoughts"}
          </h1>
        ...
        {(category || tag) && (
          <div className="mb-8 flex items-center gap-3">
            ...
            <button
              ...
              className="text-sm text-slate-500 hover:text-slate-300 underline ml-2"
            >
              Clear all
            </button>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {loading ? (
            [1, 2, 3, 4, 5, 6].map((i) => (
              <div
                key={i}
                className="h-80 w-full bg-slate-800/50 rounded-lg animate-pulse"
              />
            ))
          ) : blogs.length === 0 ? (
            ...
            blogs.map((blog) => (
              <div
                key={blog._id}
                className="group flex flex-col items-start justify-between rounded-xl border border-slate-700/50 bg-slate-800/20 hover:bg-slate-800/40 transition p-4"
              >
